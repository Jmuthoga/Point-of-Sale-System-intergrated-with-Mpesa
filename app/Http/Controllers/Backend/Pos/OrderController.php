...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // 🔒 Check if any product in the cart has stock <= 10
        foreach ($carts as $cart) {
            if ($cart->product->quantity <= 10) {
                return response()->json([
                    'message' => "Cannot complete the sale. Product '{$cart->product->name}' has low stock ({$cart->product->quantity} left). Minimum required is more than 10."
                ], 422);
            }
        }
    ...
    public function collection(Request $request, $id)
    {
        ...
        if ($request->isMethod('post')) {
            $data = $request->validate([
                ...
                'paid_by' => 'required|in:cash,mpesa,bank', 
            ]);
